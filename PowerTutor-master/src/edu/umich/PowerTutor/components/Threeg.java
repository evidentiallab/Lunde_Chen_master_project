...
public class Threeg extends PowerComponent {
  ...
  public Threeg(Context context, PhoneConstants phoneConstants) {
    ...
    log3G("interfaceName: " + interfaceName);
    ...
    log3G("transPacketsFile: " + "/sys/devices/virtual/net/" + interfaceName + "/statistics/tx_packets");
    readPacketsFile = "/sys/devices/virtual/net/" +
                      interfaceName + "/statistics/rx_packets";
    log3G("readPacketsFile: " + "/sys/devices/virtual/net/" + interfaceName + "/statistics/rx_packets");
    readBytesFile = "/sys/devices/virtual/net/" +
                    interfaceName + "/statistics/rx_bytes";
    log3G("readBytesFile: " + "/sys/devices/virtual/net/" + interfaceName + "/statistics/rx_bytes");
    transBytesFile = "/sys/devices/virtual/net/" +
                     interfaceName + "/statistics/tx_bytes";
    log3G("transBytesFile: " + "/sys/devices/virtual/net/" + interfaceName + "/statistics/tx_bytes");
  ...
  public IterationData calculateIteration(long iteration) {
    ...
    //  Log.w(TAG, "Failed to read packet and byte counts from wifi interface");
    	log3G("Failed to read packet and byte counts from wifi interface");
      return result;
    }
    ...
    if(lastUids != null) for(int uid : lastUids) {
      ...
      try {
        ...
        if(receiveBytes == -1 || transmitBytes == -1) {
          ...
          log3G("Failed to read uid read/write byte counts");
        ...
        log3G("Non-uid files in /proc/uid_stat");
      }
    }
  ...
  private static class ThreegStateKeeper {
    ...
    public void interfaceOff() {
      ...
      powerState = POWER_STATE_IDLE;
      log3G("interfaceOff()");
      log3G("powerState = POWER_STATE_IDLE");
    }
    ...
    public void updateState(long transmitPackets, long receivePackets,
      ...
      log3G("SystemClock.elapsedRealtime(): " + curTime);
      if(lastTime != -1 && curTime > lastTime) {
        ...
        switch(powerState) {
          case POWER_STATE_IDLE:
        	 log3G("POWER_STATE_IDLE");
            if(!inactive) {
              powerState = POWER_STATE_FACH;
              log3G("POWER_STATE_FACH");
            }
            break;
          ...
          default: // case POWER_STATE_DCH:
        	  log3G("POWER_STATE: default");
            if(inactive) {
              stateTime++;
              log3G("stateTime " + stateTime);
      ...
      lastTime = curTime;
      log3G("lastTime " + curTime);
      lastTransmitPackets = transmitPackets;
      log3G("lastTransmitPackets " + lastTransmitPackets);
      lastReceivePackets = receivePackets;
      log3G("lastReceivePackets " + lastReceivePackets);
      lastTransmitBytes = transmitBytes;
      log3G("lastTransmitBytes " + lastTransmitBytes);
      lastReceiveBytes = receiveBytes;
      log3G("lastReceiveBytes " + lastReceiveBytes);
    }
