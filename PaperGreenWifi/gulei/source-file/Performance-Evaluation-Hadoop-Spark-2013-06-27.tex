...
\title{Memory or Time: Performance Evaluation for Hadoop and Spark}
...
\begin{abstract}
%\boldmath
Currently, there exist various cluster computing frameworks supporting large-scale data-intensive applications \deleted[remark={delete repeated words}]{on commodity clusters}. \added[remark={Separate Hadoop and Spark and explain why Hadoop is not good for iterative operations}]{Hadoop, an open source implementation of the MapReduce programming model, is a very popular general purpose framework for many different classes of applications. However, Hadoop is not good for iterative operations because of data reloading from disk at each iteration. Spark, an emerging framework, is best suited for iterative operations due to its feature of data cached in memory across machines of cluster in the whole iterative process.} We have conducted some experiments to compare the \replaced[remark={replace "iterative performance" with "system performance for iterative algorithms"}]{system performance for iterative algorithms}{iterative performance} between Hadoop and Spark. From our experimental results, we find that Spark is faster than Hadoop in iterative operations, but it is memory consuming and its speed advantage is weakened \replaced[remark={replace "by insufficient memory" with "at the moment when the memory is not sufficient enough to store newly created intermediate results" to make it more accurate}]{at the moment when the memory is not sufficient enough to store newly created intermediate results}{by insufficient memory}.
\end{abstract}
% Benefited from its characteristics of data cached in memory across machines of cluster

% no keywords
\begin{IEEEkeywords}
\added[remark={5-10 keywords are required}]{System performance, Hadoop, Spark, PageRank, running time, memory usage}
\end{IEEEkeywords}
...
\section{\added[remark={rewrite this section to make it more logical}]{Introduction}}
% no \IEEEPARstart
The rapid development of the Internet has generated vast amounts of data which poses great challenges to traditional data processing model. To deal with such challenges, a variety of cluster computing frameworks have been proposed to support large-scale data-intensive applications on commodity machines. MapReduce, introduced by Google\cite{jdean2004} is one such framework for processing large data sets in a scalable, reliable and fault-tolerant manner. 
...
However, Hadoop is not good for iterative operations which are very common in many applications. MapReduce cannot keep reused data and state information during execution. Thus, MapReduce reads the same data iteratively and materializes intermediate results in local disks in each iteration, requiring lots of disk accesses, I/Os and unnecessary computations. Spark \cite{url_spark} is a MapReduce-like cluster computing framework \cite{matei2010}, which is designed to overcome Hadoop's shortages in iterative operations. Spark introduces a data structure called resilient distributed datasets(RDDs)\cite{matei2012}, through which reused data and intermediate results can be cached in memory across machines of cluster in the whole iterative process. This feature is best suited for low-latency iterative jobs.


Although Spark can bring tremendous speedup, we suspect its memory cost behind this huge performance boost. In this paper we evaluate the system performance for iterative algorithms between Hadoop and Spark by choosing a typical iterative algorithm PageRank. We implemented the PageRank algorithm according to the programming models provided by Hadoop and Spark respectively and applied the algorithm to different sizes of graph datasets to compare their running time and memory usage. 

\added[remark={add experimental results}]{Experimental results show that 1) Spark can outperform Hadoop by 10x-15x in Spark's comfort zone which means there is enough memory for the whole iterative process; 2) However, Spark is memory consuming. Although input datasets are not that large compared to the whole memory of the cluster, intermediate results generated at each iteration can easily fill the whole memory of the cluster; 3) Performance advantage of Spark is weakened to be almost the same as and even worse than Hadoop by insufficient memory.}



\section{\added[remark={This is a newly added section. 1. Briefly introduce Hadoop 2. Intrdouce Spark and explain why Spark designed the RDD data structure  3. Compare Hadoop and Spark in several aspects.}]{System Overview}}
...
Finally, we compare Hadoop and Spark in the following aspects.
  \begin{itemize}
...
The experimental cluster is composed of eight computers. One of them is designated as master, and the other seven as slaves. We use the operating system Ubuntu 12.04.2(GNU/Linux 3.5.0-28-generic x86 64) for all the computers.

Table \ref{tab:machine_info} shows the hostname, machine modal, IP address, CPU and memory information of the eight computers mentioned above. All of the eight computers are in the same Local Area Network and connected by a H3C S5100 switch whose port rate is 100Mbps. In order to facilitate description, we will use hostnames to reference cluster computers. All of the eight computers have 4 cores, but master and slaves have different frequencies. From the table we can also see that the master has more total amount of memory than slaves, and that the amount of free memory of each host is about 3400 MB.
...
Apache Mesos is a cluster manager that provides efficient resource isolation and sharing across distributed frameworks. It can run Hadoop, Spark, MPI, Hypertable and other frameworks. We use Mesos to manage the resources of our experimental cluster. Figure \ref{fig:cluster-architec} shows the architecture of the experimental cluster. Each slave in the experimental cluster contributes 4 CPU cores and 3072 MB(i.e, 3GB) memory to Mesos. So the total resources managed by Mesos are 28 CPU cores and 21 GB memory. When a job is submitted to Hadoop or Spark, Mesos will allocate resources to it according to its needs. 

We use Mesos 0.9.0, Hadoop 0.20.205.0 and Spark 0.6.1 for all the experiments below. The reason why we choose Mesos 0.9.0 and Spark 0.6.1 is that they are relatively new stable release. Although Hadoop 1.1.2 is the most recent stable release as of writing this paper, only Hadoop 0.20.205.0 is ported to run on mesos. \replaced[remark={the reason why we choose Hadoop 0.20.205.0 is that we want both Hadoop and Spark run over Mesos, so they can have the same experimental conditions, i.e., 28 CPU cores and 21 GB memory.}]{We choose Hadoop 0.20.205.0 in order to let Spark and Hadoop share the same experimental conditions.}{We have to choose Hadoop 0.20.205.0 in order to make Spark and Hadoop have the same experimental conditions.} We also install Hadoop 1.1.2 separately to repeat the experiment, and find that the new version Hadoop does have performance improvement and the improvement is a constant under different graph datasets.  
...
%In order to observe these two properties more intuitively, we plot them in Figure \ref{fig:node_edge_number}. From Figure \ref{fig:node_edge_number} we can see that the node numbers gradually increases from wiki-Vote to kronecker2 except web-Google which is slighly bigger than its subsequent web-BerkStan and the edge numbers exhibit exponential increasing trend from wiki-vote to kronecker2.
...
\hline
19.txt & 40 MB & 416,962 & 3,206,497 & Synthetic graph generated by Kronecker generator
 with 19 iterations.\\
\hline
20.txt & 89MB & 833,566 & 7,054,294 & Synthetic graph generated by Kronecker generator
 with 20 iterations.\\
\hline
21.txt & 209MB & 1,665,554 & 15,519,448 & Synthetic graph generated by Kronecker generator
 with 21 iterations.\\
\hline
22.txt & 479MB & 3,330,326 & 34,142,787 & Synthetic graph generated by Kronecker generator
 with 22 iterations.\\
\hline
23.txt & 1.1GB & 6,654,956 & 75,114,133 & Synthetic graph generated by Kronecker generator
 with 23 iterations.\\
\hline
\end{tabular}
...
\subsection{\added[remark={delete unneccessary content}]{PageRank Overview}}
...
\begin{equation}
P(n) = \sum_{m \in L(n)} \frac{P(m)}{C(m)} \label{eq:pagerank_simple}
\end{equation}
where $L(n)$ is the set of nodes that link to $n$, $P(m)$ is the PageRank of node $m$, $C(m)$ is the out-degree of node $m$.

%Formula \ref{eq:pagerank_simple} is actually a simplified version of PageRank definition and is only suited for a strongly connected graph. However, graphs in real world are not that perfect. So, the simplified version of PageRank needs to be modified to deal with some special structures of real graphs.
...
To deal with the first problem, a random jump factor $\alpha$ can be added to the Formula \ref{eq:pagerank_simple}. According to \cite{monica2005}, the second problem can be solved by redistributing PageRank mass ``lost'' at dangling nodes across all nodes in the graph evenly.

% With probability $\alpha$ the random surfer executes a random jump, and with probability $(1 - \alpha)$ the random surfer follows an outlink. Formula \ref{eq:pagerank_1} gives solution to the first problem. 
% \begin{equation}
% P(n) = \alpha (\frac{1}{|G|}) + (1 - \alpha) \sum_{m \in L(n)} \frac{P(m)}{C(m)} \label{eq:pagerank_1}
% \end{equation}
% where $|G|$ is the total number of nodes in the graph, $\alpha$ is the random jump factor.The random jump factor $\alpha$ is sometimes called the ``teleportation'' factor; alternatively, $(1 - \alpha)$ is referred to as the ``damping'' factor.

% This redistribution process can be accomplished by iterate over all nodes again. At the same time, we can take into account the random jump factor. 
For node $n$, its current PageRank value $P(n)$ is updated to the final PageRank value $P(n)'$ according to the following formula:
\begin{equation}
P(n)' = \alpha (\frac{1}{|G|}) + (1 - \alpha) (\frac{m}{|G|} + P(n)) \label{eq:pagerank_2}
\end{equation}
where $|G|$ is the total number of nodes in the graph, $\alpha$ is the random jump factor, $m$ is the PageRank mass ``lost'' at dangling nodes.


\subsection{\added[remark={delete the algorithm implementation on Hadoop, because our implementation references other's implementation.}]{PageRank on Hadoop}}
PageRank implementation on Hadoop is well-known. Our PageRank implementation on Hadoop mainly reference the algorithm in\cite{jimmy2010}. Due to space limitations, we will not present the PageRank algorithm on Hadoop.

The basic process of each PageRank iteration can be divided into two phases each of which corresponds to a MapReduce job. We call the first phase phase 1 and the second phase 2.
...
\subsection{\added[remark={Point out our implementation on Spark is different from the Spark authors'. }]{PageRank on Spark}}
The authors who invented Spark provide a simple implementation of PageRank on Spark. But their implementation did not consider the dangling nodes in the graph. There are actually a large number of dangling nodes in both real and synthetic graphs. Dealing with dangling nodes is a necessary step, so we reimplement PageRank on Spark by taking into account the dangling nodes in order to make the PageRank implementation on Hadoop and Spark be consistent and practical.
...
%   case (node, (neighbors, rank)) $=>$ neighbors.map(neighbor $=>$ (neighbor, rank/neighbors.size))
% \})
...
% Ensuring that there are no inherent bottlenecks as algorithms are applied to increasingly larger datasets. The gold standard, of course is linear scalability, an algorithm running on twice the amount of data should take only twice as long. Similarly, an algorithm running on twice the number of nodes should only take half as long.
In this subsection, we study the scalability of our PageRank implementations on Hadoop and Spark.

\begin{figure*}[!t]
    \centering
      \subfloat[Real Graphs]{       
        \includegraphics[width=0.5\textwidth]{figures/real-nodes-time.eps}
        }
      \subfloat[Synthetic Graphs]{        
        \includegraphics[width=0.5\textwidth]{figures/synthetic-nodes-time.eps}}
...
For each dataset we make PageRank run five times on both Hadoop and Spark, and record the time each dataset spends. Figure \ref{fig:time_vs_size} shows running time versus number of nodes for real graphs and synthetic graphs.

We mainly have 2 observations for this reuslt. 1) When datasets are smaller than 22.txt, our PageRank implementations exhibit near-linear scalability on both Hadoop and Spark whether the graphs are real or synthetic. 2) When datasets are bigger than 22.txt, slope of Spark line is greater than that of Hadoop line which means that the performance advantage of Spark becomes smaller as the graph size increases.

\subsection{Running Time Comparison}
In this subsection, we compare the running time between Hadoop and Spark.

\begin{figure*}[!t]
\centering
\subfloat[Real Graphs]{
  \includegraphics[width=0.5\textwidth]{figures/real-5iterations-time.eps}}
\subfloat[Synthetic Graphs]{
  \includegraphics[width=0.5\textwidth]{figures/synthetic-5iterations-time.eps}}
...
For each dataset, we make both implementations of PageRank iterate 5 times and record time spent. Figure \ref{fig:hadoopsparkrunningtime} shows running time comparison between Hadoop and Spark under real graphs and synthetic graphs.

We mainly have 5 observations for this result. 
...
2) When dataset is small, e.g., web-Google, 19.txt, 20.txt, Spark can outperform Hadoop by 10x-15x. In this situation, the computing resources of all the slaves are moderately utilized. The iterative computation time begins to dominate the total iteration time of Hadoop and Hadoop starts to show its true performance.

3) When dataset is large, e.g., cit-Patents, 22.txt, the advantage of Spark becomes small and Spark can only outperform Hadoop by 3x-5x. In this situation, for Hadoop the memory of each slave in the cluster is still moderately utilized while for Spark the memory of each slave in the cluster is full(i.e. reaching the memory limit of each slave in the cluster) which result in Spark's reduced performance advantage.

4) When dataset is too large, e.g., 23.txt, Hadoop beats Spark. In this situation, for Hadoop the memory of each slave in the cluster is fully utilized while for Spark the memory of each slave in the cluster is not enough for PageRank running. There is no memory space for the newly created RDDs, so Spark starts its RDD replacement policy. Frequent RDD replacements cause Spark's performance to degrade to that of Hadoop. Even worse.  
...
% In order to eliminate the randomness of a single running of PageRank on Hadoop and Spark, we repeat 10 times running of PageRank on both platforms against three sizes of datasets and get averaged total running times. Figure \ref{fig:iter10avgtime} shows the averaged value comparison of Hadoop and Spark.

\subsection{Memory Usage Comparison}
In this subsection, we compare the memory usage between Hadoop and Spark. We also use five iterations for all the datasets. 

When PageRank is running on either Hadoop or Spark, we query and record each slave's system memory usage every second. When it ends, we can plot the temporal change of memory usage for each slave according to the records we have kept. By comparing the plots we find that the memory usage plots of all the seven slaves are roughly the same under the same size of dataset. This is because of the uniform distribution of the input data on the seven slaves and also illustrates the load balancing strategy adopted by both platforms. So we will only target at one slave(say, slave1) in the analysis below. Figure \ref{fig:time_memory_real} and Figure \ref{fig:time_memory_syn} show the memory usage.
...
\begin{figure*}[!t]
  \centering
  \subfloat[Hadoop]{    
    \includegraphics[width=0.5\textwidth]{figures/hadoop-real-nodes-time-memory.eps}}
  \subfloat[Spark]{ 
    \includegraphics[width=0.5\textwidth]{figures/spark-real-nodes-time-memory.eps}}
...
\begin{figure*}[!t]
  \centering
  \subfloat[Hadoop]{    
    \includegraphics[width=0.5\textwidth]{figures/hadoop-synthetic-nodes-time-memory.eps}}
  \subfloat[Spark]{ 
    \includegraphics[width=0.5\textwidth]{figures/spark-synthetic-nodes-time-memory.eps}}
...
Now we analyze and explain the results shown above. 
 

For Hadoop platform, when dataset is fixed, memory usage exhibits periodically increase and decrease with the increase in the number of iterations. There is also a gradual increment between two consecutive iterations. The periodicity is due to the iterative nature of PageRank algorithm. Each cycle corresponds to one iteration of PageRank. In each cycle, there are three peaks, which corresponds to the Mapper and Reducer stages in phase 1 and the Mapper stage in phase 2 respectively. All these three stages consume memory and lead to a peak of memory usage. The Mapper stage in phase 1 comsumes a little bit more memory than its corresponding Reducer stage in phase 1 and Mapper stage in phase 2. Reducer stage in phase 1 lasts longer than the other two stages. 
...
% \bfseries Name & \bfseries Hadoop & \bfseries Spark & \bfseries Ratio\\
% \hline
% 19.txt & 379s & 30s & 12.6\\
% \hline
% 20.txt & 457s & 55s & 8.3\\
% \hline
% 21.txt & 637s & 117s & 5.4\\
% \hline
% 22.txt & 1040s & 311s & 3.3\\
% \hline
% 23.txt & 1481s & 1888s & 0.78\\
% \hline
% \end{tabular}
...
\section{Conclusion}
In this section, we give conclusion of our experiment.
