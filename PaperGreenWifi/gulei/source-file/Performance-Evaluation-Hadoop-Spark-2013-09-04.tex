...
\;\;\;
\begin{algorithmic}[1]
\Function {Reduce }{$n$,$<ContribList, NeighborList>$}  
  \State $p \leftarrow 0$
  \ForAll {$c \in ContribList$}
...
\label{algo:spark_pr}
\begin{algorithmic}[1]
\State val N = ...  {\small /*number of nodes in the graph*/}
\State val links = ... {\small /*RDD of (node, neighbors) pairs*/}
\State var ranks = ... {\small /*RDD of (node, rank) pairs*/}
...
\State val contribs = links .join ( ranks ) .flatMap (\{ \par
  ...
  \})  {\small /*PageRank distribution*/}
\State ranks = contribs .reduceByKey ( $\_ + \_$ ) \par {\small /*PageRank aggregation*/}
\State dMass = ranks .reduce ( \par
  \hskip\algorithmicindent ( $p1$, $p2$ ) $=>$ ( "dm", $p1.\_2 + p2.\_2$ ) \par
  ) .\_2
\State lMass = 1 - dMass {\small /*PageRank loss computation*/}
...
As a new programming model, MapReduce  \cite{jdean2004} is suitable for processing and generating large data sets in a scalable, reliable and fault-tolerant manner. Apache Hadoop\cite{url_hadoop} provides an open source implementation of MapReduce. Performance of MapReduce has been deeply studied in \cite{jiang2010}\cite{lee2012}. The authors show that with proper implementation, performance of MapReduce can approach traditional parallel databases while achieving scalability, flexibility and fault-tolerance. %However, Hadoop shows poor performance in iterative operations which are very common in many data analysis techniques.
