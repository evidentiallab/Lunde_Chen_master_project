...
public class CsvToData {
	...
	public static final String INPUT_CSV =  ROOT_DIR 
			+ "chenlunde20140907_download_cencus_gov.csv";
...
//				
//			}
			if(count >= 20 * 5000 && count <= 50 * 5000)
			{
				
			}
			else if(count >= 90 * 5000 && count <= 107 * 5000)
			{
			...
			else if(count <= 3 * 5000)
			{
				...
				if(count % 50 == 0)
				{
					writeIntoFile(OUTPUT_RESULT, tok.nextToken());
				}
			count ++;
		}
		br.close();
